...
export async function saveReadingProgress(params: {
  ...
  const { email, name, correct, total, byType, streak, passageId, mastered, leaves, passageTitle, examStyle } = params;
  if (!email || !db) return false;
  try {
    const ref = doc(db, COLLECTION, emailToId(email));
    const weekId = currentWeekId();
    ...
    const leavesChanged = !!passageId && (thisLeaves > 0 || !!prevLeaves[passageId]);

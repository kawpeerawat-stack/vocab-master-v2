...
export function review(card: SrsCard | undefined, correct: boolean): SrsCard {
  const isFirstEverAttempt = !card; // ยังไม่เคยมีการ์ดมาก่อน = คำใหม่แกะกล่อง เจอครั้งแรกในชีวิต
  ...
  if (correct) {
    c.reps += 1;
    c.streak += 1;
    // เจอครั้งแรกก็ตอบถูกเลย -> ผ่านทันที (เหมือนเดิม)
    // แต่ถ้าเคยตอบผิดมาก่อน (การ์ดนี้เคยถูกทบทวนมาแล้ว) ต้องตอบถูก "ติดต่อกัน 2 ครั้ง" ถึงจะผ่าน
    //   เพื่อยืนยันว่าจำได้จริง ไม่ใช่แค่เดาถูกพอดีครั้งเดียว
    if (isFirstEverAttempt || c.streak >= 2) {
      c.box = MAX_BOX;
    }
    // (ถ้าเคยผิดมาก่อนแล้วเพิ่งตอบถูกเป็นครั้งแรก (streak=1) -> box ยังคงเป็น 0 ต่อไป
    //  ระบบจะดึงคำนี้กลับมาถามซ้ำเร็ว ๆ นี้ เพื่อขอคำตอบถูกอีกครั้งยืนยัน)
  } else {
...
export function computeStats(store: SrsStore, allWords: string[]): SrsStats {
  ...
  // ตั้งแต่เปลี่ยนเป็น "ตอบถูก = ผ่านทันที" กล่องของแต่ละคำเหลือแค่ 0 (ยังไม่ผ่าน) หรือ 5 (ผ่านแล้ว)
  //   ไม่ได้ไล่ระดับทีละขั้นแบบเดิมอีกต่อไป — สูตรเก่าที่ผสม coverage 50% + มาสเตอรี่ 50% จึงทำให้
  //   % พุ่งเกินจริงตั้งแต่ทำไปไม่กี่คำ (เช่น ทำ 20/1000 คำ กลับโชว์ ~40%)
  // ตอนนี้ % จึงมาจาก "จำนวนคำที่ผ่านแล้วจริง ÷ คำทั้งหมด" ตรง ๆ — สอดคล้องกับคำศัพท์ที่ทำจริง
  //   (คำที่ตอบผิดค้างอยู่ยังไม่นับ เพราะยังไม่ผ่าน ต้องกลับมาตอบถูกให้ครบก่อนถึงจะนับ)
  const weightedProgress = total > 0 ? (mastered / total) * 100 : 0;
  const masteryRatio = seen > 0 ? boxSum / (MAX_BOX * seen) : 0; // เก็บไว้เป็นข้อมูลย่อย (ความแม่นในคำที่ทำไปแล้ว)
  return {
    total,
    seen,
